...
addressProviderContract = w3.eth.contract(address=Web3.to_checksum_address(
    ADDRESS_PROVIDER_CONTRACT_ADDRESS), abi=addressProviderABI)
...
lendingPoolContract = w3.eth.contract(address=Web3.to_checksum_address(
    lendingPoolContractAddress), abi=lendingPoolABI)
dataProviderContract = w3.eth.contract(address=Web3.to_checksum_address(
    dataProviderContractAddress), abi=dataProviderABI)

liquidationLoanContract = w3.eth.contract(address=Web3.to_checksum_address(
    os.getenv('LIQUIDATION_CONTRACT_ADDRESS')), abi=liquidateLoanABI)
...
print("Using bot deployed at address: ",
      os.getenv('LIQUIDATION_CONTRACT_ADDRESS'))
...
def getLiquidatablePositions():
    ...
    if response.status_code == 200:
        ...
        for liquidatablePositionData in data:
            liquidatablePositions.append(
                LiquidatablePosition(liquidatablePositionData))
    else:
        # Request failed
...
class LiquidatablePosition:
    ...
    def __str__(self):
        borrowed_assets_str = "\n".join(
            [str(asset) for asset in self.borrowedAssets])
        supplied_assets_str = "\n".join(
            [str(asset) for asset in self.supliedAssets])
...
def trigger_logic():
    ...
    for liquidatablePosition in liquidatablePositions:
        ...
        liquidatablePosition.supliedAssets = [
            asset for asset in liquidatablePosition.supliedAssets if asset.isCollateral == True]
        # iterate over the different collaterals to liquidate
        done = False
        for c in liquidatablePosition.supliedAssets:
            print(
                "=================== NEW LIQUIDATION DETECTED ==============================")
            ...
            for b in liquidatablePosition.borrowedAssets:
                ...
                liquidationBonus = dataProviderContract.functions.getReserveConfigurationData(
                    collateralAddress).call()[3] / 100 - 100
                print(f"Liquidation bonus: {liquidationBonus}")

                liquidationScaleFactor = 1 + liquidationBonus * \
                    (1 + liquidationProtocolFee / 100) / 100
                ...
                if (float(b.variableRate.fiatAmount) * liquidationScaleFactor) < float(c.fiatAmount):
                    print(
                        "Scaled Debt is less than collateral. We can liquidate full debt")
                    ...
                    debtToCover = float(c.fiatAmount) / \
                        oneDebtInUSD / liquidationScaleFactor
                    ...
                    debtToCover = int(
                        int(debtToCover * pow(10, dDecimals)) * margin)
                ...
                try:
                    ...
                    print(
                        f"Succesfully liquidated user: {liquidatablePosition.address}")
                    # finish for loop
                    done = True
                    break
